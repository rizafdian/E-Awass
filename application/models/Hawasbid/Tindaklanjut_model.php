...
class Tindaklanjut_model extends CI_Model
{
    ...
    {
        $this->db->select('tbl_temuan.*, tbl_objek.id AS id_objek');
        $this->db->from('tbl_temuan');
        $this->db->join('tbl_checklist', 'tbl_checklist.id= tbl_temuan.id_checklist');
        $this->db->join('tbl_pertanyaan','tbl_pertanyaan.id_pertanyaan = tbl_checklist.id_pertanyaan');
        $this->db->join('tbl_subobjek','tbl_pertanyaan.id_subobjek = tbl_subobjek.id_subobjek');
        $this->db->join('tbl_objek','tbl_objek.id = tbl_subobjek.id_objek');
        $this->db->where('tbl_checklist.id_pengawasan', $id);
